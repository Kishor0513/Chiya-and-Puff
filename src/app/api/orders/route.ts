...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!table || table.qrData !== token) {
            return NextResponse.json({ error: 'Invalid table or QR token' }, { status: 403 });
        }
        ...
        for (const item of items) {
            ...
            if (!dbItem || !dbItem.available) {
                return NextResponse.json({ error: `Item ${item.menuItemId} is not available` }, { status: 400 });
            }
...
export async function GET(request: NextRequest) {
    // Fetch overall orders for metrics if needed
    try {
